...
TEST_F(TestRecordBatch, ToTensorSupportedNullToNan) {
  ...
  std::vector<double> f_values = {
      static_cast<double>(NAN), 2,  3,  4,  5, 6, 7, 8, 9, 10, 20, 30, 40,
      static_cast<double>(NAN), 60, 70, 80, 90};
  auto data = Buffer::Wrap(f_values);

  std::shared_ptr<Tensor> tensor_expected;
  ASSERT_OK_AND_ASSIGN(tensor_expected, Tensor::Make(float64(), data, shape, f_strides));
